...
const Wheel = () => {
    ...
    const spinWheel = () => {
        ...
        setTimeout(() => {
            ...
            const targetCenterAngle = (targetSegment - 1) * sectionDegree + sectionDegree / 2;
    ...
    return (
        <div className="wheel-container">
            <div className="wheel" style={{ transform: `rotate(${rotation}deg)`, transition: 'transform 5s ease-out' }}>
                <svg width="300" height="300" viewBox="0 0 300 300">
                    ...
                    {Array.from({ length: 6 }).map((_, index) => {
                        ...
                        return (
                            <g key={index}>
                                <path
                                    d={`M150,150 L${150 + 140 * Math.cos((Math.PI * (-90 + index * 60)) / 180)},${
                                        150 + 140 * Math.sin((Math.PI * (-90 + index * 60)) / 180)
                                    } A140,140 0 0,1 ${
                                        150 + 140 * Math.cos((Math.PI * (-90 + (index + 1) * 60)) / 180)
                                    },${150 + 140 * Math.sin((Math.PI * (-90 + (index + 1) * 60)) / 180)} Z`}
                                    fill="black"
                                    stroke="white"
                                    strokeWidth="1"
                                />
