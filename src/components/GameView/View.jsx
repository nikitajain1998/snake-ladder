...
function View (){
  ...
  const calculateTop = (position) => {
    ...
    const top = Math.floor(row / 2) * cellSize * 2 + (rowDirection === 1 ? 0 : cellSize);
    return top;
};
 const calculateLeft = (position) => {
    const col = (position -1) % boardWidth;
    const colDirection = Math.floor((position - 1) / boardWidth) % 2 === 0 ? 1 : -1;
    const left = col * cellSize * colDirection + (colDirection === 1 ? 0 : (boardWidth - 1) * cellSize);
    return left;
};
...
const getLabelPosition = (index) => {
    ...
    return(
        <div className="gameView">
          ...
          <Player leftPosition={leftPosition} bottomPosition={topPosition} margin={margin}/> 
          <Board />
        </div>);
};
export default View;